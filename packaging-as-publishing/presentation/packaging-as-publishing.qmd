...
__🤷 "I'm not convinced. My work is only for _[ internal | quick | throwaway ]_ use so packaging / publishing for others doesn't matter."__ 
...
<center>![](2023-08-31-13-04-18.png)</center>
...
<center>![](2023-08-31-13-34-16.png)</center>
...
Code without an environment specification is difficult to run in a consistent way.
This can lead to "works on my machine" scenarios where different things happen for different people, reducing the chance that people can connect with your code.

- Using Python environment and packaging management can decouple a project from OS-level dependencies (in an image specification, for example) and opens the opportunity for it to become a dependency of another project (enabling reuse).
...
A brief history of Python environment and packaging tooling:

1. `distutils`, `easy_install` + `setup.py` (primarily used during 1990's - early 2000's)
1. `pip`, `setup.py` + `requirements.txt` (primarily used during late 2000's - early 2010's)
1. `poetry` + `pyproject.toml` (began use around late 2010's - ongoing)
1. Greater compatibility surrounding `pyproject.toml` (early 2020's)

------------------------------------------------------------------------

## Python Packaging - Environments

![](2023-08-31-13-59-31.png)
